...
void HelpScene::buildScene()
{
    ...
    LabelTTF* helpTextLabel = LabelTTF::create("", "Arial", SCALED_VALUE(20.0f));
    helpTextLabel->setDimensions(Size(AppDelegate::SCREEN_WIDTH * 0.60, AppDelegate::SCREEN_HEIGHT * 0.72));
    
    helpTextLabel->setString(helpText);
    
    this->addChild(helpTextLabel);
    helpTextLabel->setAnchorPoint(Point(0.0, 1.0));
    helpTextLabel->setVerticalAlignment(TextVAlignment::TOP);
    float py = 1 - (1 - 0.764);
